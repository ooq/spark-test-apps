 ...
 */
@DeveloperApi
class TaskBalanceListener(conf: SparkConf) extends SparkListener with Logging {
  ...
  def getSizesOfActiveStateTrackingCollections: Map[String, Int] = {
    Map(
  ...
  def getSizesOfHardSizeLimitedCollections: Map[String, Int] = {
    Map(
  ...
  def getSizesOfSoftSizeLimitedCollections: Map[String, Int] = {
    Map(
  ...
  override def onStageSubmitted(stageSubmitted: SparkListenerStageSubmitted): Unit = synchronized {
    ...
    val poolName = Option(stageSubmitted.properties).map {
      p => p.getProperty("spark.scheduler.pool", "default")
    }.getOrElse("default")
   ...
   */
  private def waitUntilExecutorsUp(numExecutors: Int, timeout: Long): Unit = {
